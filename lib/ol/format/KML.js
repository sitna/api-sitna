...
const fillSharedSchemas = async function (features) {
    for (let f of features) {
        ...
        if (featureTypeMetadata) {
            ...
            let schemaId = featureSchemaUrls.get(f);
            if (schemaId) {
                schemaId.substr(1);
            }
            else {
                schemaId = f._wrap.parent.layer?.id;
                featureSchemaUrls.set(f, '#' + schemaId);
            }
            this.sharedSchemas_[schemaId] = featureTypeMetadata.attributes.map((attr) => {
 ...
 */
    ...
    getFeatureTypeMetadata(metadata, _features) {
        return function (feature) {
            const newMetadata = { ...metadata };
            const schemaUrl = featureSchemaUrls.get(feature.wrap.feature);
            if (schemaUrl) {
                ...
                if (schema) {
                    ...
                    feature.wrap.feature.unset(schemaUrlPropertyName);
                }
            }
            return newMetadata;
        };
