...
import DragAndDrop, { DragAndDropEvent } from '../../../node_modules/ol/interaction/DragAndDrop';
        ...
        if (features && features.length > 0)
            ...
            if (features.every(a => Array.isArray(a))) {
                groups = features.reduce(function (rv, x) {
                    ...
                    const groups = /(?<layer>[-_\p{Letter}]+)?\.*(?<id>[\d]*)?/iu.exec(x[0].getId()).groups;
                    const id = groups["layer"] ?
                        groups["layer"] :
                        x[0].getGeometry() ?
                            Object.keys(ol.geom).find(type => x[0].getGeometry() instanceof ol.geom[type]) :
                            file.name;
                ...
                groups = features.reduce(function (rv, x) {
                    ...
                    if (!id) {
                        const groups = /(?<layer>[-_\p{Letter}.]+[\p{Letter}])?\.*(?<id>[\d]*)?/iu.exec(x.getId()).groups;
                        id = groups["layer"] ?
                            groups["layer"] :
                            x.getGeometry() ?
                                Object.keys(ol.geom).find(type => x.getGeometry() instanceof ol.geom[type]) :
                                file.name;
                    }
                    (rv[id] = rv[id] || []).push(x);
                    return rv;
